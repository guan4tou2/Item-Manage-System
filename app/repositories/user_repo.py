...
def get_notification_settings(username: str) -> Dict[str, Any]:
    ...
    user = mongo.db.user.find_one(
        {"User": username},
        {"email": 1, "notify_enabled": 1, "notify_days": 1, "notify_time": 1, "last_notification_date": 1}
    )
    if user:
        return {
...
def get_all_users_for_notification() -> List[Dict[str, Any]]:
    ...
    return list(mongo.db.user.find(
        ...
        {"_id": 0, "User": 1, "email": 1, "notify_days": 1, "notify_time": 1, "last_notification_date": 1}
    ))
